...
> 취업용 포트폴리오 정리 문서. 각 기능을 **문제상황 → 트레이드오프 → 기능 → 구현** 순서로 설명하고,
> 화면에 영향을 주는 기능은 **동작 전/후 스크린샷**으로 흐름을 함께 보여준다.
> 코드 라인 번호는 작성 시점(2026-09) 기준이며, 서술은 문서가 아닌 **실제 코드**를 기준으로 한다.
...
|---|---|---|
| ![로그인 폼](./screenshots/01-auth/01-a-login-form.jpg) | ![로그인 성공](./screenshots/01-auth/01-b-login-success-home.jpg) | ![보호 페이지](./screenshots/01-auth/01-c-protected-page-access.jpg) |
...
|---|---|---|
| ![전체 목록](./screenshots/03-semantic-search/03-a-plain-list.jpg) | ![겉옷 검색](./screenshots/03-semantic-search/03-b-query-outerwear.jpg) | ![가방 검색](./screenshots/03-semantic-search/03-c-query-bag.jpg) |
...
|---|---|
| ![등록 폼](./screenshots/04-embedding-async/04-a2-register-form-filled.jpg) | ![검색 노출](./screenshots/04-embedding-async/04-b-new-item-in-search.jpg) |
...
|---|---|---|
| ![판매내역](./screenshots/05-order-state/05-c-seller-requested-and-paid.jpg) | ![발송 처리](./screenshots/05-order-state/05-e-seller-after-ship.jpg) | ![거래완료](./screenshots/05-order-state/05-g-buyer-completed.jpg) |
...
|---|---|---|
| ![입력](./screenshots/08-chat/08-a-buyer-typing.jpg) | ![전송](./screenshots/08-chat/08-b-buyer-message-sent.jpg) | ![대화](./screenshots/08-chat/08-c-conversation-with-offer.jpg) |
...
|---|---|---|
| ![제안 수신](./screenshots/09-offer-order/09-b-seller-offer-accept-reject.jpg) | ![수락 후](./screenshots/09-offer-order/09-d-buyer-after-accept-pay-cta.jpg) | ![자동 거절](./screenshots/09-offer-order/09-e-competing-offer-auto-rejected.jpg) |
...
| 협의가 결제 |
|---|
| ![협의가 결제](./screenshots/09-offer-order/09-f-order-checkout-agreed-price.jpg) |
...
| 시드 직후 홈 |
|---|
| ![시드 홈](./screenshots/15-browse-ux/15-a-home-all.jpg) |
...
|---|---|---|
| ![Actions 목록](./screenshots/13-cicd/13-a-actions-list.jpg) | ![실행 요약](./screenshots/13-cicd/13-b-run-summary.jpg) | ![Job 스텝](./screenshots/13-cicd/13-c-job-steps.jpg) |
...
|---|---|---|
| ![전체](./screenshots/15-browse-ux/15-a-home-all.jpg) | ![가방](./screenshots/15-browse-ux/15-b-category-bag.jpg) | ![끝](./screenshots/15-browse-ux/15-c-infinite-scroll-end.jpg) |
...
- 로그아웃 후 Access Token 즉시 무효화(Redis `jti`), 로그인 실패 rate limiting
- 폐기된 Refresh Token 정리 배치
- `ItemService.update`(PATCH)가 비관적 락을 우회해 상태를 바꿀 수 있는 경로
- 스케줄러 분산 락(현재 단일 인스턴스 전제)
- 프레임워크 예외(Security 인증 실패, 누락 파라미터, `DataIntegrityViolationException`)까지 `{error, message}` 통일
- STOMP 개인 에러 큐(`/user/queue/errors`)
- 도메인별 상세 개선 방향은 [`docs/`](.)의 `*-review.md` 문서 참고
