...
- **에러 응답이 `{error, message}` JSON 포맷으로 전환 완료** — `GlobalExceptionHandler`가 공통 부모 `ApplicationException`(+ `ErrorCode`)을 기준으로 `ErrorResponse(error, message)`를 반환하도록 회원 도메인을 포함한 전 도메인이 함께 리팩터링됐다(`MemberException`→`not_found_member`, `DuplicateMemberException`→`duplicate_member`/`duplicate_nickname`). `check-id`/`check-nickname`의 `MissingServletRequestParameterException`은 여전히 별도 핸들러가 없어 Spring 기본 에러 페이지로 나가는 점은 남은 이슈다.
...
### 6. Address(임베디드 값 타입)를 API 계약 그대로 노출
...
6. (우선순위 낮음) Address를 API 전용 DTO로 감싸기, 죽은 코드/미사용 import 정리.
</content>
